...
async function test() {
  ...
  try {
    console.log("Fetching journal from invoice...");
    const invoice = await odooCall(creds, uid, 'account.move', 'read', [[record.invoiceId]], { fields: ['journal_id'] });
    const journalId = invoice[0].journal_id[0];
    console.log("Found Journal ID:", journalId);

    console.log("Step 1: Create Reversal Wizard with explicit journal_id...");
